import { Home, BarChart3, PieChart, Settings, ChevronRight } from 'lucide-react'

const sections = [
  {
    title: 'Dashboard',
    items: [
      { icon: Home, label: 'Company Internal Comparison' },
    ],
  },
  {
    title: 'Analytics',
    items: [
      { icon: BarChart3, label: 'Fund Risk Metrics Analysis' },
      { icon: PieChart, label: 'Sector Risk Analysis' },
    ],
  },
  {
    title: 'System',
  ...
  return (
    <aside className="hidden lg:flex flex-col w-72 bg-[#0c0f14]/90 backdrop-blur supports-[backdrop-filter]:bg-[#0c0f14]/80 border-r border-white/10 text-white">
      <div className="h-20 flex items-center gap-3 px-6 border-b border-white/10">
        ...
        <div>
          <p className="font-semibold leading-tight">Finicify</p>
          <p className="text-xs text-white/60">Financial Analytics Dashboard</p>
        </div>
      </div>
